<template>
    ...
    <div class="myrow" style="text-align: center;">
      <div v-for="cat in this.cats" :key="cat.id"  @click="this.goToCat(cat.basecat,cat.cat,cat.subcat)" class="flex xs6 md4" >
        <div class="boxshop1" style="background: white;margin: 10px">
          <img v-if="cat.pic===''" src="/src/assets/images/nopic.png" style="height: 22px;max-width:22px;border-radius: 11px;margin-top: 3px; ">
  ...
  <div  v-if="this.geted===2 && this.products.length>0" style="margin: 2% 3% 0 3%">
    <div  v-for="item in products" :key="item.id" @click="this.goToPage(item.id)" class="mycard" style="height: 120px;width:100%;margin-top: 10px;cursor: pointer">
      <div class="rowshop" style="padding: unset">
        ...
        <div style="width:100%">
          <p style="height:65px;padding: 20px 2% 10px 0;font-size: 14px;color: var(--va-basecolor);font-weight: bold ">{{item.name}}</p>
  ...
  <div v-else-if="this.geted===2 && this.products.length===0" style="margin-top: 150px">
    <div class="row" >
    ...
    <div class="row" style="width: 100%;text-align: center;">
      <p class="default" style="width: 100%;text-align: center">هیچ محصولی در این دسته وجود ندارد</p>
    </div>
  </div>
  <FooterShop :selected_num="3" />
</template>
